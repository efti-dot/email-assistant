...
├── evaluation_results.json       # Model A full report (generated by evaluate.py)
├── evaluation_results.csv        # Model A scores (generated by evaluate.py)
...
Prompting Technique (Advanced Requirement)

This assistant uses a combined Role-Playing + Chain-of-Thought + Per-Tone Guide prompt (see prompts.py):
...
Chain-of-Thought forces the model to reason through intent → facts → tone → structure before drafting, which improves fact recall and tone accuracy.
...
bashgit clone https://github.com/your-username/email-assistant.git
cd email-assistant
...
bashpython evaluate_second_model.py
...
MetricTypeWhat it measuresfact_recallRule-based% of supplied key facts whose keywords appear in the outputtone_accuracyLLM-as-JudgeGPT-4o-mini rates 1–5 how well writing style matches requested toneconciseness_clarityHybridAverage of length-ratio score vs reference email + LLM clarity rating
